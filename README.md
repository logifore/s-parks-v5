# S-parks V4 Prototype

S-parks V4 是基于 `sparks-v3.5` 升级出的独立静态原型版本，重点是在保留原有功能闭环和交互可用性的前提下，引入 `v4_ui设计风格` 中更强的画报感、编辑式布局和创作者工作台气质。

## V4 重点

- 保留 V3.5 的路由、数据结构、状态管理和核心交互能力，避免功能退化。
- 首页、创作者页、支持中心、素材陈列区改为更鲜明的 V4 视觉语言。
- 继续保留搜索、筛选、素材详情、时间切换、收藏、购买授权、下载中心、上传流程、审核后台等原型功能。
- 通过新增样式覆盖和渲染结构升级，尽量减少对底层逻辑的破坏范围。
...
V4 仍是纯前端原型，不包含 API key、环境变量、后端接口或真实用户数据。
